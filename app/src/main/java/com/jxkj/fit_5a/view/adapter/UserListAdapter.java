 ...
 * date   : 2020/5/2914:03
 */
public class UserListAdapter extends BaseQuickAdapter<FollowFansList.DataBean.UserBean, BaseViewHolder> {
    public UserListAdapter(@Nullable List<FollowFansList.DataBean.UserBean> data) {
        super(R.layout.item_user_gz, data);
    }

    @Override
    protected void convert(@NonNull BaseViewHolder helper, FollowFansList.DataBean.UserBean user) {
        if(user==null){
            return;
        }
        ...
        helper.setText(R.id.tv_user_name,user.getNickName()).setText(R.id.tv_time, "");
        if(user.getGender()==1){
