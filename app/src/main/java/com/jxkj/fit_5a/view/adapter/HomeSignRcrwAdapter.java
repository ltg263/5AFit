...
//        helper.setProgress(R.id.pb_progressbar,(int)item.getCurCount(),(int)item.getCount());
        helper.setText(R.id.tv_title,item.getName());
        if(item.getStatus()==1){
            helper.setText(R.id.tv_go_sign,"去完成");
        ...
        if(item.getSpeeds()!=null && item.getSpeeds().size()>0){
            ...
            helper.setText(R.id.tv_num,speeds.getSpeed()+"/"+speeds.getTarget());//+speeds.getUnit()
