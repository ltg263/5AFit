 ...
 * date   : 2020/5/2914:03
 */
public class MineCommentAdapter_gf extends BaseQuickAdapter<CommentMomentBean, BaseViewHolder> {
    int contentType = 2;
    ...
    public MineCommentAdapter_gf(@Nullable List<CommentMomentBean> data) {
    ...
    protected void convert(@NonNull BaseViewHolder helper, CommentMomentBean item) {
        ...
        MineComment2Adapter_gf mMineComment2Adapter_gf = new MineComment2Adapter_gf(null);
        RvList.setLayoutManager(new LinearLayoutManager(mContext));
        RvList.setHasFixedSize(true);
        RvList.setAdapter(mMineComment2Adapter_gf);

        mMineComment2Adapter_gf.setOnItemChildClickListener(new OnItemChildClickListener() {
            @Override
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                switch (view.getId()) {
                    case R.id.iv_liuyan2:
                        CommentMomentBean data = mMineComment2Adapter_gf.getData().get(position);
                        showSoftInputFromWindow(mMineComment2Adapter_gf,view,data.getCommentId(),item, "回复@" + data.getUser().getNickName() + ":");
                        break;
                }
            }
        });
        ...
        if(item.getCommentCount()>0){
            ...
            HttpRequestUtils.getCommentQueryReply_gf(item.getCommentId()+"", item.getMomentId()+"", item.getMomentPublisherId()+"", 1, 1, new HttpRequestUtils.ResultInterface() {
                @Override
                public void succeed(ResultList<CommentMomentBean> result) {
                    if(result!=null && result.getCode()==0 && result.getData().size()>0){
                        mMineComment2Adapter_gf.setNewData(result.getData());
                    }
                }
            });
        }
        ...
        helper.getView(R.id.tv_num).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                HttpRequestUtils.getCommentQueryReply_gf(item.getCommentId(), item.getMomentId(), item.getMomentPublisherId(), 1, 100, new HttpRequestUtils.ResultInterface() {
                    @Override
                    public void succeed(ResultList<CommentMomentBean> result) {
     ...
     * EditText获取焦点并显示软键盘
     */
    public void showSoftInputFromWindow(MineComment2Adapter_gf mMineComment2Adapter_gf, View view, String commentId, CommentMomentBean item, String context2) {
        //使用自定义的dialog
        dialog_input = new SoftKeyboardInputDialog(mContext, context2, R.style.DialogTheme,
                new SoftKeyboardInputDialog.DialogInterfaceKey() {
                    ...
                    public void strContext(String context) {
                        liuyan(mMineComment2Adapter_gf,context,commentId,item);
                    }
                });
        dialog_input.show();
    ...
    private void liuyan(MineComment2Adapter_gf mMineComment2Adapter_gf, String context, String commentId, CommentMomentBean item) {
        HttpRequestUtils.postCommentMoment_gf(context, item.getMomentId(), item.getMomentPublisherId(),
                        ...
                        new Thread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                ((Activity)mContext).runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        HttpRequestUtils.getCommentQueryReply_gf(item.getCommentId(), item.getMomentId(),
