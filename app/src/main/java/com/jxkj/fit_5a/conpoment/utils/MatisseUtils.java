...
public class MatisseUtils {
    ...
    public static void gotoSelectVideo(Activity mActivity) {
        if (EasyPermissions.hasPermissions(mActivity, params)) {
            //具备权限 直接进行操作
            MatisseUtils.selectImgVideo(mActivity);
        } else {
            //权限拒绝 申请权限
            EasyPermissions.requestPermissions(mActivity, "为了您更好使用本应用，请允许应用获取以下权限", PERMISSION_CAMERA, params);
        }
    ...
    public static void selectImgVideo(Activity mActivity) {
    ...
    public static void oPenUrlImgL(Activity mActivity,List<LocalMedia> lists,int pos){
        PictureSelector.create(mActivity).themeStyle(R.style.picture_default_style)
                .imageEngine(GlideEngine.createGlideEngine())
                .openExternalPreview(pos, lists);
    }
}
